...
arr = (list(map(int, input().split()))[1:] for _ in range(a))
print(max(map(lambda x: sum(i ** 2 for i in x) % b, product(*arr))))
...
#     result = max(sum([x * x for x in combination]) % b, result)
# print(result)
